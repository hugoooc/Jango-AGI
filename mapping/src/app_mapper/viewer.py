...
def build_viewer_data(
    ...
    workspace_ids = [node["node_id"] for node in nodes if node["state_type"] == "workspace"]
    depths = {workspace_ids[0]: 0} if workspace_ids else {}
    for _ in range(len(nodes)):
        changed = False
        for edge in edges:
            source_depth = depths.get(edge["source_node_id"])
            if source_depth is None or edge["destination_node_id"] in depths:
                continue
            depths[edge["destination_node_id"]] = source_depth + 1
            changed = True
        if not changed:
            break
    return {
...
<main><section id="metrics" class="metrics"></section><section class="workspace"><div class="panel"><h2>Map <span class="muted">— concentric rings represent graph depth</span></h2><svg id="graph" viewBox="0 0 1200 1000" role="img" aria-label="Directed OpenVSP navigation graph"></svg></div><aside id="detail" class="panel"><h2>Selection details</h2><p class="muted">Select a state in the map.</p></aside></section><section id="lists" class="lists"></section></main>
...
const metricLabels={mapped_nodes:'Mapped states',directed_edges:'Directed edges',graph_depth:'Graph depth',replay_reliability:'Replay reliability',safe_frontier_coverage:'Safe frontier mapped',recorded_edges:'Evidence-backed edges'};
...
const center=D.nodes.find(n=>n.state_type==='workspace')||D.nodes[0],pos={},depth={};if(center){pos[center.node_id]=[600,500];depth[center.node_id]=0}for(let pass=0;pass<D.nodes.length;pass++){let changed=false;D.edges.forEach(e=>{if(depth[e.source_node_id]!=null&&depth[e.destination_node_id]==null){depth[e.destination_node_id]=depth[e.source_node_id]+1;changed=true}});if(!changed)break}const maxDepth=Math.max(1,...Object.values(depth));for(let d=1;d<=maxDepth;d++){const level=D.nodes.filter(n=>depth[n.node_id]===d),radius=d===1?350:465;level.forEach((n,i)=>{const a=(Math.PI*2*i/Math.max(level.length,1))-Math.PI/2;pos[n.node_id]=[600+radius*Math.cos(a),500+radius*Math.sin(a)]})}D.nodes.filter(n=>pos[n.node_id]==null).forEach((n,i,a)=>{const angle=Math.PI*2*i/Math.max(a.length,1);pos[n.node_id]=[600+475*Math.cos(angle),500+475*Math.sin(angle)]});
...
D.edges.forEach(e=>{const a=pos[e.source_node_id],b=pos[e.destination_node_id];if(!a||!b)return;const dx=b[0]-a[0],dy=b[1]-a[1],len=Math.hypot(dx,dy)||1,ox=dx/len*35,oy=dy/len*35;const p=el('path',{d:`M ${a[0]+ox} ${a[1]+oy} L ${b[0]-ox} ${b[1]-oy}`,'marker-end':'url(#arrow)',class:'edge '+(e.replay.failures?'bad':'')});p.style.cursor='pointer';p.onclick=()=>showEdge(e);svg.appendChild(p)});
D.nodes.forEach(n=>{const [x,y]=pos[n.node_id],g=el('g',{class:'node '+n.state_type,transform:`translate(${x} ${y})`});g.appendChild(el('circle',{r:31}));const label=el('text',{y:-2});label.textContent=n.semantic_name.length>14?n.semantic_name.slice(0,13)+'…':n.semantic_name;g.appendChild(label);const type=el('text',{y:12});type.textContent=n.state_type;type.setAttribute('fill','#98a9b9');g.appendChild(type);g.onclick=()=>showNode(n);svg.appendChild(g)});
